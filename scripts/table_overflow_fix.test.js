...
(async () => {
  ...
  try {
    ...
    if (!cols) bad('جدول المخزون ما ظهرش خالص');
      ...
      ok(`٦ أعمدة ظاهرة على الموبايل بدل ٨ (${cols.join(', ')})`);
      if (cols.includes('الفئة') || cols.includes('مباع')) bad('الأعمدة الثانوية لسه ظاهرة على الموبايل');
      else ok('الفئة ومباع مختفيين على الموبايل، وهيرجعوا على الديسك توب');
    }
